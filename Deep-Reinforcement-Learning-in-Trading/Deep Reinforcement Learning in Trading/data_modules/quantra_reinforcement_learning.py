...
def normalize_ftmo_transaction_cost(ftmo_cost, entry, lots=1.0):
    """Normalize a fixed FTMO round-trip cost into a proportional cost.
    This assumes ftmo_cost is the round-trip fee in account currency for the trade.
    """
    if entry == 0 or lots == 0:
        return 0.0
    return abs(ftmo_cost) / (abs(entry) * abs(lots))
...
def reward_pos_log_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
    ...
    pnl = get_pnl(entry, curr, pos, ftmo_cost, lots)
    if pnl >= 0:
...
def reward_pure_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
    '''pure pnl'''
    return get_pnl(entry, curr, pos, ftmo_cost, lots)


def reward_positive_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
...
def reward_categorical_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
    '''Sign of pnl'''
    return np.sign(get_pnl(entry, curr, pos, ftmo_cost, lots))


def reward_positive_categorical_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
...
def reward_exponential_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
    '''exp pure pnl'''
    return np.exp(get_pnl(entry, curr, pos, ftmo_cost, lots))


def reward_ftmo_pnl(entry, curr, pos, ftmo_cost=0.001, lots=1.0):
    '''FTMO-style reward using net PnL after fixed transaction cost'''
    return get_pnl(entry, curr, pos, ftmo_cost, lots)
...
class Game(object):
    ...
    def __init__(self, bars5m, bars1d, bars1h, reward_function, lkbk=20, init_idx=None,
                 ftmo_cost=0.001, ftmo_lot=1.0, max_drawdown=None,
                 max_loss_per_trade=None, max_trade_len=None):
        self.bars5m = bars5m
        self.lkbk = lkbk
        self.trade_len = 0
        self.stop_pnl = None
        ...
        self.ftmo_cost = ftmo_cost
        self.ftmo_lot = ftmo_lot
    ...
    def act(self, action):
        ...
        if self.position != 0 and self.max_loss_per_trade is not None:
            if self.pnl <= -abs(self.max_loss_per_trade):
                self.is_over = True

        if not self.is_over and self.position != 0 and self.max_trade_len is not None:
            if self.curr_idx - self.start_idx >= self.max_trade_len:
                self.is_over = True
...
def init_net(env, rl_config):
    ...
    modelQ.compile(SGD(learning_rate=rl_config['LEARNING_RATE']), loss=rl_config['LOSS_FUNCTION'])
    ...
    modelR.compile(SGD(learning_rate=rl_config['LEARNING_RATE']), loss=rl_config['LOSS_FUNCTION'])
...
def run(bars5m, rl_config):
    ...
    env = Game(bars5m, bars1d, bars1h, rl_config['RF'],
               ...
               ftmo_cost=ftmo_cost, ftmo_lot=ftmo_lot,
    ...
    while True:
        ...
        while not game_over:
            ...
            tl = pd.DataFrame([[env.curr_time, env.position, episode]], columns=["curr_time", "position", "episode"])
            if game_over:
                tl = pd.DataFrame([[env.curr_time, 0, episode]], columns=["curr_time", "position", "episode"])
            trade_logs=pd.concat([trade_logs,tl],ignore_index=True)
        ...
        if max_drawdown is not None:
            ...
            if drawdown <= -abs(max_drawdown):
                print('Max drawdown threshold reached ({:.2f}%). Stopping further trading.'.format(drawdown*100))
                break
